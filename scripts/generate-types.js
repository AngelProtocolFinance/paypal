...
const MODULE_NAMES = {
  ...
  'payment-experience_web_experience_profiles_v1.json': 'web-experience-profiles',
...
async function generateTypes(specFile) {
  ...
  try {
    ...
    console.error(`✗ Failed to generate types for ${moduleName}:`, error.message);
...
async function createIndexFile(modules) {
  const exports = modules
    .map(moduleName => `export * as ${moduleName.replace(/-/g, '_')} from './${moduleName}';`)
    .join('\n');
